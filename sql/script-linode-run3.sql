...
-- Metrics from raw prometheus DB
-- **** containers
...
select 'service_ltcy_sum' as name, date_trunc('seconds',  time::timestamp) as ztime
,left(labels ->> 'destination_service', strpos(labels ->> 'destination_service', '.') - 1) service, value
from metrics 
where name = 'istio_request_duration_seconds_sum'
and labels ->> 'destination_service_namespace' = 'default'
and labels ->> 'response_code' = '200'
and (  labels ->> 'destination_service' like 'checkoutservice%'
--	or labels ->> 'destination_service' like 'cartservice%'	
...
-- service ltcy count
select 'service_ltcy_count' as name, date_trunc('seconds',  time::timestamp) as ztime
,left(labels ->> 'destination_service', strpos(labels ->> 'destination_service', '.') - 1) service, value
from metrics 
...
select metric_name, labels from metrics_labels 
where metric_name like 'istio_request_duration_seconds_sum' fetch first 3 rows only;

-- why not ltc via these buckets instead
select 'service_ltcy' as name, date_trunc('seconds',  time::timestamp) as ztime
,left(labels ->> 'destination_service', strpos(labels ->> 'destination_service', '.') - 1) service, value
from metrics
where name = 'istio_request_duration_seconds_bucket'
...
and labels ->> 'response_code' = '200'
and labels ->> 'le' = '+Inf'
and labels ->> 'destination_service_name' = 'checkoutservice'
and time between '2020-02-27 22:49:50'::timestamp and '2020-02-28 20:44:00'::timestamp 
...
select 'system_network_receive' as name, date_trunc('seconds',  time::timestamp) as ztime
, left(labels ->> 'instance', strpos(labels ->> 'instance', ':') - 1) as node, value 
from metrics 
where name = 'node_network_receive_bytes_total' 
;

select 'system_network_transmit' as name, date_trunc('seconds',  time::timestamp) as ztime
, left(labels ->> 'instance', strpos(labels ->> 'instance', ':') - 1) as node, value 
from metrics 
